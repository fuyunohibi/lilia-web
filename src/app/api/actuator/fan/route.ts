...
let lastKnownFanState: Record<string, "on" | "off"> = {};
...
export async function POST(request: NextRequest) {
  ...
  const topic = `esp32/${device_id}/fan`;
  mqttClient.publish(topic, command);
  lastKnownFanState[device_id] = command;
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ state: lastKnownFanState[device_id] ?? "off" });
}
